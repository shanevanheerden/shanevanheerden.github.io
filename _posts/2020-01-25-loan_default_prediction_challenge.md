...
where `prev_default_loan`$=\mbox{SUM}($`default_loan`$)$ is a new feature indicating the total number of loans a customer has previously defaulted on and $`loan_number` is the loan number of the current loan. The interest rate charged to each customer is defined mathematically as
...
where `prev_term_days`$=\mbox{AVG}($`term_days`$)$ is a new feature indicating the average term days offered to the customer on their previous loans. And finally, a new feature `prop_loan_amount` is constructed to indicate the proportion of customer's loan amount on their current and previous loans, defined mathematically as
...
where $`prev_loan_amount`$=\mbox{AVG}($`loan_amount`$)$ is a new feature indicating a customer's average previous loan amounts.
