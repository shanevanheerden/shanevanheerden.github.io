...
The split criterion employed by the C4.5 algorithm is based on Shannon's well-known concept of entropy from information theory<d-cite key="Shannon1949"></d-cite>. Entropy is a measure of the disorder in a data subset. Denote the set of $C_j$ categories for attribute $j$ by $\mathcal{C}_j=c_1$ ... $c_{C_j}^{(j)}$, and the frequency of observations in a particular subset of data $\mathcal{S}\subset\mathcal{X}$ with a target variable of $q$ by freq$(q,\mathcal{S})$. Then the entropy of a category $c_k^{(j)}$ of attribute $j$ is given by
